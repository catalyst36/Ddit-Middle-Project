...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#btn-reply").on('click', function(){
			...
			location.href="<%=request.getContextPath()%>/comment.do?reply="+reply+"&cmt_code="+cmt_code+"&bor_code=<%=bor_code%>&std_id=<%=std_id%>&nav=질문";
		});
		...
		$(document).on('click','.cmt-choice',function(){
			...
			$.ajax({
				...
				success : function(resData){
					console.log("hello");
					location.href="/studditproject/boardDetail.do?code="+<%=bor_code%>+"&nav=질문";
				},
				error : function(xhr){
					alert("상태 : " + xhr.status);
				}
		...
		$('#boardDelete').on('click',function(){
			alert("삭제가 완료되었습니다.");
			location.href="<%=request.getContextPath()%>/deleteBoard.do?code=<%=boardVo.getBor_code()%>&nav=질문";
		});
	});
				...
				<div class="content">
					...
					<div class="content-footer">
						<div class="button-wrapper">
							<button onclick="location.href='<%=request.getContextPath()%>/help/help.jsp'" class="btn btn-outline-secondary">돌아가기</button>
