...
@WebServlet("/studyMainBoardJSP.do")
public class StudyMainBoardJSP extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//한글 처리
				...
				//service메소드 호출하기
				List<StudyMainBoardVO> list = service.jspList();
				//request에 결과 데이터 저장
				request.setAttribute("studyBoardAllList", list);//jsp쪽이랑 일치시켜라.
				
				//페이지로 이동
				request.getRequestDispatcher("main/studyMainBoardJSP.jsp").forward(request, response);
