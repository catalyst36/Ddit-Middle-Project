 ...
 */
var currentPage = 1;
listPageServer = function(cst, area ,tit, word, search, cpage){
	$.ajax({
		url : '/studditproject/boardList.do',
		type : 'get',
		data  : {
			"cst" : cst,
			"type-area" : area,
		...
		success : function(resData){
			console.log(cst ,area, tit, word, search, cpage);
			let navCode = '<ul class="nav nav-tabs me-auto mb-2 mb-lg-0">';
			navCode += '<li class="nav-item dropdown">';
			navCode += '<a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">모집중</a>';
			...
			navCode += '</ul>';
			navCode += '</li>';
			navCode += '<li class="nav-item">';
			
			if(cst == '진행'){
				navCode += '<a class="nav-link move-item nav-sel active" aria-current="page" href="#">진행</a>';
			} else{
				navCode += '<a class="nav-link move-item nav-sel" aria-current="page" href="#">진행</a>';
			}
			
			navCode += '</li>';
			navCode += '<li class="nav-item">';
			if(cst == '마감'){
				navCode += '<a class="nav-link move-item nav-sel active" href="#">마감</a>';
			} else{
				navCode += '<a class="nav-link move-item nav-sel" href="#">마감</a>';
			}
			navCode += '</li>';	
			navCode += '</ul>';	
...
commentList = function(code, reply, id){
	$.ajax({
