...
<script type="text/javascript">
	...
	$(function(){
		...
		$(document).on('click','#btn-previous',function(){
			...
			let sct = "모집";
			if(word == '전체') word = "";
			else if(word == '진행' || word == '마감') cst = word;
			else {
				area = "bor_area";
			}
			if(search != "") {
			...
			listPageServer(cst, area, tit, word, search, currentPage);
		});
		...
		$(document).on('click','.move-detail', function(){
			...
			let word = $('.active').first().text();
			if(word == '진행'){
				location.href="/studditproject/myStudyDetail.do?code="+code+"&any=1";
			}else if(word == '마감'){
				location.href="/studditproject/myStudyDetail.do?code="+code+"&any=2";
			}else {
				location.href="/studditproject/boardDetail.do?code="+code;
			}
