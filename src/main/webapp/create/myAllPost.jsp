				...
				<table class="table">
					...
					%>
						<tr class="move-detail">
							<th scope="row"><%=mypost.get(i).getBor_code() %></th>
							<td><%=mypost.get(i).getBor_name() %></td>
							<td><%=mypost.get(i).getBor_tit() %></a></td>
							<td><%=mypost.get(i).getBor_rpd()%></td>
						<!-- 	<td><a href="<%=request.getContextPath()%>/myStudyDetail.do?code=<%=mypost.get(i).getBor_code()%>"><%=mypost.get(i).getBor_tit() %></a></td> -->
						</tr>
					<%		
						}
					%>
